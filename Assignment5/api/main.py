...
@app.put("/resources/{resource_id}", response_model=schemas.Recipe, tags=["Recipes"])
def update_one_resource(resource_id: int, resource: schemas.RecipeUpdate, db: Session = Depends(get_db)):
...
@app.delete("/resources/{resource_id}", tags=["Recipes"])
